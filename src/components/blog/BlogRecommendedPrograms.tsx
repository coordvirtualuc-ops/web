...
// ─── Selección determinista de 3 programas ────────────────────────────────────
// Usamos el día del año para rotar la selección sin Math.random en render.
// Así es estable durante todo el día en Server Component.

function getThreePrograms() {
  const validPrograms = academicPrograms.filter(
    (p) => p.id && p.status !== "coming-soon"
  )
  if (validPrograms.length <= 3) return validPrograms.slice(0, 3)
  ...
  // Tomar 3 consecutivos con wrap-around
  const indices = [0, 1, 2].map((i) => (dayOfYear + i) % validPrograms.length)
  return indices.map((i) => validPrograms[i])
}

// ─── BlogRecommendedPrograms ──────────────────────────────────────────────────

export function BlogRecommendedPrograms() {
  const programs = getThreePrograms()

  if (!programs.length) return null

  return (
    <section
      id="programas-sugeridos"
      className="bg-[#F8F5F2] py-16 md:py-24"
      aria-labelledby="programas-sugeridos-titulo"
    >
          ...
          {programs.map((program) => {
            ...
            return (
              ...
              >
                ...
                <div className="h-1.5 bg-gradient-to-r from-[#550f2a] to-[#8c002a]" aria-hidden="true" />

                <div className="flex flex-col flex-1 p-6">
                  {/* Icono + categoría */}
                  <div className="flex items-center gap-2 mb-4">
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#550f2a]/8 shrink-0">
                      <GraduationCap className="h-5 w-5 text-[var(--color-wine)]" aria-hidden="true" />
                    </div>
                  ...
                  {/* Descripción */}
                  <p className="text-sm text-[var(--color-text)]/65 leading-relaxed mb-5 flex-1">
